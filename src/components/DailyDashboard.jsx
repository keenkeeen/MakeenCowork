...
import { getUsers, updateUser, calculateRemainingDays } from '../utils/storage';
import { v4 as uuidv4 } from 'uuid';
import moment from 'jalali-moment';
import * as XLSX from 'xlsx';

export default function DailyDashboard({ users, setUsers }) {
  const [detailUser, setDetailUser] = useState(null);
  const [editMode, setEditMode] = useState(false);
  const [editForm, setEditForm] = useState({});
  const [snackbar, setSnackbar] = useState({ open: false, message: '', severity: 'success' });
  const [breakTimers, setBreakTimers] = useState({}); // { userId: { running: bool, start: Date, total: ms } }
  const [refresh, setRefresh] = useState(0);
  const [search, setSearch] = useState('');
  const [filterForced, setFilterForced] = useState('all'); // all, forced, not-forced
  ...
  // فیلتر و سورت کاربران
  let filteredUsers = users;
  if (search) filteredUsers = filteredUsers.filter(u => (`${u.name} ${u.family}`).toLowerCase().includes(search.toLowerCase()));
  if (filterForced === 'forced') filteredUsers = filteredUsers.filter(u => u.type === 'cowork-forced');
  if (filterForced === 'not-forced') filteredUsers = filteredUsers.filter(u => u.type !== 'cowork-forced');
  ...
  function splitByAttendance(arr) {
    const absent = arr.filter(u => !(u.attendances || []).some(a => a.date === today && a.enterTime));
    const present = arr.filter(u => (u.attendances || []).some(a => a.date === today && a.enterTime));
    return [...absent, ...present];
  }
  const forced = filteredUsers.filter(u => u.type === 'cowork-forced');
  const bootcamp = filteredUsers.filter(u => u.isBootcamp && u.type !== 'cowork-forced');
  const normal = filteredUsers.filter(u => !u.isBootcamp && u.type !== 'cowork-forced');
  const sortedUsers = [
    ...splitByAttendance(forced),
    ...splitByAttendance(bootcamp),
    ...splitByAttendance(normal)
  ];

  // محاسبه مجموع ساعت‌های ورود/خروج
  ...
  // وضعیت ورود/خروج امروز
  const getTodayAttendance = (user) => {
    const today = new Date().toISOString().slice(0, 10);
    return (user.attendances || []).find(a => a.date === today);
  };
  // وضعیت خروج موقت باز امروز
  const getOpenBreak = (user) => {
    const today = new Date().toISOString().slice(0, 10);
    return (user.outSessions || []).find(s => s.date === today && s.start && !s.end);
  };

  // تغییر وضعیت cowork-forced
  const handleToggleForced = (user) => {
    if (!user.isBootcamp) {
      setSnackbar({ open: true, message: 'این کاربر دانشجوی بوتکمپ نیست', severity: 'error' });
      return;
    }
  ...
  const handleCheckInOut = (user) => {
    ...
    const usersList = users.map(u => {
      ...
      let att = (u.attendances || []).find(a => a.date === today);
      if (!att) {
        // ثبت ورود
        isCheckIn = true;
        // بررسی شارژ یا روز باقی‌مانده قبل از ثبت ورود (همه کاربران در داشبورد کوورک)
        const remainingDays = calculateRemainingDays(u);
        if (remainingDays <= 0) {
          setSnackbar({ open: true, message: 'اشتراک کاربر به پایان رسیده است. لطفاً ابتدا اشتراک را تمدید کنید.', severity: 'error' });
          found = false;
          return u;
        }
        found = true;
    ...
    setSnackbar({ open: true, message: found ? (user.attendances && user.attendances.find(a => a.date === today && !a.exitTime) ? 'خروج ثبت شد' : 'ورود ثبت شد') : 'امروز قبلاً ورود و خروج ثبت شده است', severity: 'success' });
    setRefresh(r => r + 1);
  };

  // مدیریت خروج موقت (کرنومتر)
  const handleBreak = (user) => {
    ...
    if (idx === -1) return;
    let open = (usersList[idx].outSessions || []).find(s => s.date === today && s.start && !s.end);
    if (!open) {
  // شروع خروج موقت
    usersList[idx].outSessions = [
      ...(usersList[idx].outSessions || []),
  ...
  const exportToExcel = () => {
    const data = sortedUsers.map((user, i) => {
      ...
      return {
        ...
        'نوع کاربر': user.type === 'cowork-forced' ? 'کوورک اجباری' : user.isBootcamp ? 'دانشجو' : 'عادی',
    ...
    XLSX.writeFile(wb, 'users_dashboard.xlsx');
  };
    ...
    <Box sx={{ my: 4 }}>
      ...
      <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2} mb={3} alignItems="center">
        ...
        <FormControl size="small" sx={{ minWidth: 160 }}>
          <InputLabel>فیلتر کوورک اجباری</InputLabel>
          <Select
            value={filterForced}
            label="فیلتر کوورک اجباری"
            onChange={e => setFilterForced(e.target.value)}
          >
            <MenuItem value="all">همه</MenuItem>
            <MenuItem value="forced">فقط کوورک اجباری</MenuItem>
            <MenuItem value="not-forced">غیر اجباری</MenuItem>
          </Select>
        </FormControl>
        <FormControl size="small" sx={{ minWidth: 160 }}>
      ...
      {/* جدول کاربران */}
      <Typography variant="h5" mb={3} align="center">داشبورد کاربران</Typography>
